...
import type { CrudState } from "./state";
import type { Emitter } from "./hooks";

import { ElMessage, ElMessageBox } from "element-plus";
import { cloneDeep, get, omitBy, overSome, isNil, snakeCase } from "lodash-unified";
import { sleep, to } from "@yusui/utils";

export type CrudMethods = ReturnType<typeof useCrudMethods>;
...
export function useCrudMethods<T extends Data, P extends Data>({
   ...
   * @param {Function} loading 为表单停止loading函数
   */
  const handleUpdate = async (row: T, index?: number, done?: () => void, loading?: () => void) => {
   ...
   */
  const searchChange = async (form: P, done: () => void) => {
   ...
   */
  const sortChange: AvueCrudProps["onSortChange"] = ({ order, prop }) => {
    if (order && prop) {
   ...
   */
  const beforeOpen = async (done: () => void, type: FormType) => {
   ...
   * @param type 表单类型 add|edit|view
   */
  const beforeClose = async (done: () => void, type: FormType) => {
